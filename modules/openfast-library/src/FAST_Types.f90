...
! =========  FAST_MiscVarType  =======
  TYPE, PUBLIC :: FAST_MiscVarType
    ...
    TYPE(FileInfoType)  :: ADskInlineFileInfo      !< serialized inline AeroDisk input from a YAML primary file (YAML text lines with provenance) [-]
    LOGICAL  :: ADskIsInline = .FALSE.      !< AeroDisk input was given inline in the YAML primary file [-]
    TYPE(FileInfoType)  :: SEDInlineFileInfo      !< serialized inline Simplified ElastoDyn input from a YAML primary file (YAML text lines with provenance) [-]
    LOGICAL  :: SEDIsInline = .FALSE.      !< Simplified ElastoDyn input was given inline in the YAML primary file [-]
    TYPE(FileInfoType)  :: EDInlineFileInfo      !< serialized inline ElastoDyn input from a YAML primary file (YAML text lines with provenance) [-]
    LOGICAL  :: EDIsInline = .FALSE.      !< ElastoDyn input was given inline in the YAML primary file [-]
    TYPE(FileInfoType)  :: SeaStInlineFileInfo      !< serialized inline SeaState input from a YAML primary file (YAML text lines with provenance) [-]
    LOGICAL  :: SeaStIsInline = .FALSE.      !< SeaState input was given inline in the YAML primary file [-]
    TYPE(FileInfoType)  :: SrvDInlineFileInfo      !< serialized inline ServoDyn input from a YAML primary file (YAML text lines with provenance) [-]
    LOGICAL  :: SrvDIsInline = .FALSE.      !< ServoDyn input was given inline in the YAML primary file [-]
    TYPE(FileInfoType)  :: HDInlineFileInfo      !< serialized inline HydroDyn input from a YAML primary file (YAML text lines with provenance) [-]
    ...
    TYPE(FileInfoType)  :: SDInlineFileInfo      !< serialized inline SubDyn input from a YAML primary file (YAML text lines with provenance) [-]
    LOGICAL  :: SDIsInline = .FALSE.      !< SubDyn input was given inline in the YAML primary file [-]
    TYPE(FileInfoType)  :: ExtPtfmInlineFileInfo      !< serialized inline ExtPtfm_MCKF input from a YAML primary file (YAML text lines with provenance) [-]
    LOGICAL  :: ExtPtfmIsInline = .FALSE.      !< ExtPtfm_MCKF input was given inline in the YAML primary file [-]
    TYPE(FileInfoType)  :: FEAMInlineFileInfo      !< serialized inline FEAMooring input from a YAML primary file (YAML text lines with provenance) [-]
    LOGICAL  :: FEAMIsInline = .FALSE.      !< FEAMooring input was given inline in the YAML primary file [-]
    TYPE(FileInfoType)  :: IceDInlineFileInfo      !< serialized inline IceDyn input from a YAML primary file (YAML text lines with provenance) [-]
    LOGICAL  :: IceDIsInline = .FALSE.      !< IceDyn input was given inline in the YAML primary file [-]
    TYPE(FileInfoType)  :: OrcaInlineFileInfo      !< serialized inline OrcaFlex input from a YAML primary file (YAML text lines with provenance) [-]
    LOGICAL  :: OrcaIsInline = .FALSE.      !< OrcaFlex input was given inline in the YAML primary file [-]
...
subroutine FAST_CopyMisc(SrcMiscData, DstMiscData, CtrlCode, ErrStat, ErrMsg)
   ...
   call NWTC_Library_CopyFileInfoType(SrcMiscData%ADskInlineFileInfo, DstMiscData%ADskInlineFileInfo, CtrlCode, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   if (ErrStat >= AbortErrLev) return
   DstMiscData%ADskIsInline = SrcMiscData%ADskIsInline
   call NWTC_Library_CopyFileInfoType(SrcMiscData%SEDInlineFileInfo, DstMiscData%SEDInlineFileInfo, CtrlCode, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   if (ErrStat >= AbortErrLev) return
   DstMiscData%SEDIsInline = SrcMiscData%SEDIsInline
   call NWTC_Library_CopyFileInfoType(SrcMiscData%EDInlineFileInfo, DstMiscData%EDInlineFileInfo, CtrlCode, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   if (ErrStat >= AbortErrLev) return
   DstMiscData%EDIsInline = SrcMiscData%EDIsInline
   call NWTC_Library_CopyFileInfoType(SrcMiscData%SeaStInlineFileInfo, DstMiscData%SeaStInlineFileInfo, CtrlCode, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   if (ErrStat >= AbortErrLev) return
   DstMiscData%SeaStIsInline = SrcMiscData%SeaStIsInline
   call NWTC_Library_CopyFileInfoType(SrcMiscData%SrvDInlineFileInfo, DstMiscData%SrvDInlineFileInfo, CtrlCode, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   if (ErrStat >= AbortErrLev) return
   DstMiscData%SrvDIsInline = SrcMiscData%SrvDIsInline
   call NWTC_Library_CopyFileInfoType(SrcMiscData%HDInlineFileInfo, DstMiscData%HDInlineFileInfo, CtrlCode, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   if (ErrStat >= AbortErrLev) return
   DstMiscData%HDIsInline = SrcMiscData%HDIsInline
   call NWTC_Library_CopyFileInfoType(SrcMiscData%ADInlineFileInfo, DstMiscData%ADInlineFileInfo, CtrlCode, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   if (ErrStat >= AbortErrLev) return
   ...
   call NWTC_Library_CopyFileInfoType(SrcMiscData%SDInlineFileInfo, DstMiscData%SDInlineFileInfo, CtrlCode, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   if (ErrStat >= AbortErrLev) return
   DstMiscData%SDIsInline = SrcMiscData%SDIsInline
   call NWTC_Library_CopyFileInfoType(SrcMiscData%ExtPtfmInlineFileInfo, DstMiscData%ExtPtfmInlineFileInfo, CtrlCode, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   if (ErrStat >= AbortErrLev) return
   DstMiscData%ExtPtfmIsInline = SrcMiscData%ExtPtfmIsInline
   call NWTC_Library_CopyFileInfoType(SrcMiscData%FEAMInlineFileInfo, DstMiscData%FEAMInlineFileInfo, CtrlCode, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   if (ErrStat >= AbortErrLev) return
   DstMiscData%FEAMIsInline = SrcMiscData%FEAMIsInline
   call NWTC_Library_CopyFileInfoType(SrcMiscData%IceDInlineFileInfo, DstMiscData%IceDInlineFileInfo, CtrlCode, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   if (ErrStat >= AbortErrLev) return
   DstMiscData%IceDIsInline = SrcMiscData%IceDIsInline
   call NWTC_Library_CopyFileInfoType(SrcMiscData%OrcaInlineFileInfo, DstMiscData%OrcaInlineFileInfo, CtrlCode, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   if (ErrStat >= AbortErrLev) return
   DstMiscData%OrcaIsInline = SrcMiscData%OrcaIsInline
end subroutine
...
subroutine FAST_DestroyMisc(MiscData, ErrStat, ErrMsg)
   ...
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   call NWTC_Library_DestroyFileInfoType(MiscData%ADskInlineFileInfo, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   call NWTC_Library_DestroyFileInfoType(MiscData%SEDInlineFileInfo, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   call NWTC_Library_DestroyFileInfoType(MiscData%EDInlineFileInfo, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   call NWTC_Library_DestroyFileInfoType(MiscData%SeaStInlineFileInfo, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   call NWTC_Library_DestroyFileInfoType(MiscData%SrvDInlineFileInfo, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   ...
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   call NWTC_Library_DestroyFileInfoType(MiscData%SDInlineFileInfo, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   call NWTC_Library_DestroyFileInfoType(MiscData%ExtPtfmInlineFileInfo, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   call NWTC_Library_DestroyFileInfoType(MiscData%FEAMInlineFileInfo, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   call NWTC_Library_DestroyFileInfoType(MiscData%IceDInlineFileInfo, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
   call NWTC_Library_DestroyFileInfoType(MiscData%OrcaInlineFileInfo, ErrStat2, ErrMsg2)
   call SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
end subroutine
...
subroutine FAST_PackMisc(RF, Indata)
   ...
   call NWTC_Library_PackFileInfoType(RF, InData%ADskInlineFileInfo) 
   call RegPack(RF, InData%ADskIsInline)
   call NWTC_Library_PackFileInfoType(RF, InData%SEDInlineFileInfo) 
   call RegPack(RF, InData%SEDIsInline)
   call NWTC_Library_PackFileInfoType(RF, InData%EDInlineFileInfo) 
   call RegPack(RF, InData%EDIsInline)
   call NWTC_Library_PackFileInfoType(RF, InData%SeaStInlineFileInfo) 
   call RegPack(RF, InData%SeaStIsInline)
   call NWTC_Library_PackFileInfoType(RF, InData%SrvDInlineFileInfo) 
   call RegPack(RF, InData%SrvDIsInline)
   call NWTC_Library_PackFileInfoType(RF, InData%HDInlineFileInfo) 
   ...
   call NWTC_Library_PackFileInfoType(RF, InData%SDInlineFileInfo) 
   call RegPack(RF, InData%SDIsInline)
   call NWTC_Library_PackFileInfoType(RF, InData%ExtPtfmInlineFileInfo) 
   call RegPack(RF, InData%ExtPtfmIsInline)
   call NWTC_Library_PackFileInfoType(RF, InData%FEAMInlineFileInfo) 
   call RegPack(RF, InData%FEAMIsInline)
   call NWTC_Library_PackFileInfoType(RF, InData%IceDInlineFileInfo) 
   call RegPack(RF, InData%IceDIsInline)
   call NWTC_Library_PackFileInfoType(RF, InData%OrcaInlineFileInfo) 
   call RegPack(RF, InData%OrcaIsInline)
   if (RegCheckErr(RF, RoutineName)) return
end subroutine
...
subroutine FAST_UnPackMisc(RF, OutData)
   ...
   call NWTC_Library_UnpackFileInfoType(RF, OutData%ADskInlineFileInfo) ! ADskInlineFileInfo 
   call RegUnpack(RF, OutData%ADskIsInline); if (RegCheckErr(RF, RoutineName)) return
   call NWTC_Library_UnpackFileInfoType(RF, OutData%SEDInlineFileInfo) ! SEDInlineFileInfo 
   call RegUnpack(RF, OutData%SEDIsInline); if (RegCheckErr(RF, RoutineName)) return
   call NWTC_Library_UnpackFileInfoType(RF, OutData%EDInlineFileInfo) ! EDInlineFileInfo 
   call RegUnpack(RF, OutData%EDIsInline); if (RegCheckErr(RF, RoutineName)) return
   call NWTC_Library_UnpackFileInfoType(RF, OutData%SeaStInlineFileInfo) ! SeaStInlineFileInfo 
   call RegUnpack(RF, OutData%SeaStIsInline); if (RegCheckErr(RF, RoutineName)) return
   call NWTC_Library_UnpackFileInfoType(RF, OutData%SrvDInlineFileInfo) ! SrvDInlineFileInfo 
   call RegUnpack(RF, OutData%SrvDIsInline); if (RegCheckErr(RF, RoutineName)) return
   call NWTC_Library_UnpackFileInfoType(RF, OutData%HDInlineFileInfo) ! HDInlineFileInfo 
   ...
   call NWTC_Library_UnpackFileInfoType(RF, OutData%SDInlineFileInfo) ! SDInlineFileInfo 
   call RegUnpack(RF, OutData%SDIsInline); if (RegCheckErr(RF, RoutineName)) return
   call NWTC_Library_UnpackFileInfoType(RF, OutData%ExtPtfmInlineFileInfo) ! ExtPtfmInlineFileInfo 
   call RegUnpack(RF, OutData%ExtPtfmIsInline); if (RegCheckErr(RF, RoutineName)) return
   call NWTC_Library_UnpackFileInfoType(RF, OutData%FEAMInlineFileInfo) ! FEAMInlineFileInfo 
   call RegUnpack(RF, OutData%FEAMIsInline); if (RegCheckErr(RF, RoutineName)) return
   call NWTC_Library_UnpackFileInfoType(RF, OutData%IceDInlineFileInfo) ! IceDInlineFileInfo 
   call RegUnpack(RF, OutData%IceDIsInline); if (RegCheckErr(RF, RoutineName)) return
   call NWTC_Library_UnpackFileInfoType(RF, OutData%OrcaInlineFileInfo) ! OrcaInlineFileInfo 
   call RegUnpack(RF, OutData%OrcaIsInline); if (RegCheckErr(RF, RoutineName)) return
end subroutine
