...
subroutine ui_quad_src_11(CP, Sigma, xi, eta, RefPoint, R_g2p, UI)
   ...
   ! Transfo in element coordinate system, noted x,y,z, but in fact xi eta zeta
   ! Manual inlining of matmul(R_g2p, DP) to avoid intrinsic issues on device
   ...
   ! Manual inlining of matmul(transpose(R_g2p), Vp) to avoid intrinsic issues on device
...
elemental real(ReKi) function signit(ref, val)
  ...
  if ( abs(val) > epsilon(1.0_ReKi) ) then
   ...
   !> Adapted from NWTC_Num:EqualRealNos
   pure function EqualRealNos_local(ReNum1, ReNum2) result(isEqual)
      !$OMP DECLARE TARGET
