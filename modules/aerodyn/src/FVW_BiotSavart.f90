...
!$OMP DECLARE TARGET
   ...
   do k = 1, 3
      UI(k) = 0.0_ReKi
      do l = 1, 3
         UI(k) = UI(k) + R_g2p(l, k) * Vp(l) ! Transpose access: R_g2p(l, k) instead of R_g2p(k, l)
      end do
   end do
...
pure function EqualRealNos_Target(ReNum1, ReNum2) result(isEqual)
!$OMP DECLARE TARGET
