...
subroutine ui_quad_src_11(CP, Sigma, xi, eta, RefPoint, R_g2p, UI)
   ...
   ! --- Tan term 
   ! 12
   if (abs(xi2-xi1) <= max(abs(xi2+xi1), 1.0_ReKi) * local_Tol) then ! Security - Hess 1962 - page 47 - bottom
      TAN12=0._ReKi
   else
   ...
   ! 23
   if (abs(xi3-xi2) <= max(abs(xi3+xi2), 1.0_ReKi) * local_Tol) then ! Security - Hess 1962 - page 47 - bottom
      TAN23=0._ReKi
   else
   ...
   ! 34
   if (abs(xi4-xi3) <= max(abs(xi4+xi3), 1.0_ReKi) * local_Tol) then ! Security - Hess 1962 - page 47 - bottom
      TAN34=0._ReKi
   else
   ...
   ! 41
   if (abs(xi1-xi4) <= max(abs(xi1+xi4), 1.0_ReKi) * local_Tol) then ! Security - Hess 1962 - page 47 - bottom
      TAN41=0._ReKi
   else
   ...
   Vp(1)= Sigma/(local_fourpi)*( (eta2-eta1)*RJ12 + (eta3-eta2)*RJ23 + (eta4-eta3)*RJ34 + (eta1-eta4)*RJ41 )
   Vp(2)= Sigma/(local_fourpi)*( (xi1-xi2)  *RJ12 + (xi2-xi3)  *RJ23 +  (xi3-xi4) *RJ34 +  (xi4-xi1) *RJ41 )
   Vp(3)= Sigma/(local_fourpi)*( ( TAN12 ) + ( TAN23 ) + ( TAN34 ) + ( TAN41 ) )
...
subroutine ui_quad_src_nn(CPs, Sigmas, xi, eta, RefPoint, R_g2p, UI, nCPs, nPanels)
   ...
   if (nCPs > 0 .and. nPanels > 0) then
      !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO MAP(to: CPs, Sigmas, RefPoint, xi, eta, R_g2p) MAP(tofrom: UI) &
...
elemental real(ReKi) function signit(ref, val)
  ...
  real(ReKi),parameter  :: local_PRECISION_EPS = epsilon(1.0_ReKi)
  if ( abs(val)>local_PRECISION_EPS ) then
      signit = sign(ref, val)
  else
      signit = 1.0_ReKi
  endif
